-- energix_final.sql
-- Fichier complet de la base de données pour l'ERP SuccessFuel
-- Inclut toutes les tables nécessaires pour les modules décrits dans projet_success_fuel.md
...
-- Table pour les spécifications locales
...
-- Table pour les configurations par pays
...
-- Table pour les tranches de taxes (si applicable)
...
-- Table pour les unités de mesure
...
-- Modules pour la gestion des droits
...
-- 1) Profils (liés à une compagnie)
...
-- Table d'association profil-permissions
...
-- 2) Stations (liée à une compagnie)
...
-- 3) Utilisateurs (dépend de profils)
...
-- 4) Familles articles (liée à un gérant)
...
-- 5) Plan comptable (liée à un gérant)
...
-- 6) type tiers (liée à une compagnie)
...
-- 7) Articles (liée à une compagnie, une famille d'articles et un plan comptable)
...
-- Table pour l'historique des prix des articles
...
-- Nouvelle table pour les carburants
...
-- Table pour l'historique des prix des carburants
...
-- 13) Cuves / Pistolets / Livraisons / Index / Cuve stocks / Bilans (liés à une compagnie via la station)
CREATE TABLE cuves (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Table pour stocker les points de barémage (hauteur/volume) d'une cuve
...
-- 14) Pompe carburants (liée à une compagnie via la cuve/station)
CREATE TABLE pompes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    station_id UUID REFERENCES stations(id) ON DELETE CASCADE,
...
-- Table pour l'historique des index des pistolets
...
-- Table pour la réinitialisation sécurisée des index de pistolets
...
-- 8) Clients & Fournisseurs (liés à une compagnie, optionnellement à une station)
...
-- 9) Employés (liés à une compagnie et optionnellement à une station)
...
-- 8) Trésoreries (liée à une compagnie, optionnellement à une station)
...
-- 9) Journal des écritures (lié à une compagnie)
...
-- 10) Achats & Détails (liés à une compagnie)
CREATE TABLE achats (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    fournisseur_id UUID REFERENCES fournisseurs(id) ON DELETE SET NULL,
...
-- Table pour les mesures de livraison (avant/après) et écart
...
-- Table pour les modifications sensibles (pour détection d'anomalies)
...
-- Table pour les politiques de sécurité
...
-- Table pour gérer les permissions spécifiques de trésorerie par utilisateur
...
-- Mise à jour de la table stocks pour inclure les données pour le CUMP
ALTER TABLE stocks
ADD COLUMN IF NOT EXISTS valeur_stock NUMERIC(18,2) DEFAULT 0,  -- Valeur totale du stock
ADD COLUMN IF NOT EXISTS cout_unitaire_moyen NUMERIC(18,4) DEFAULT 0;  -- Coût unitaire moyen (CUMP)

-- Ajouter des champs de validation aux écritures comptables
ALTER TABLE journal_entries
ADD COLUMN IF NOT EXISTS est_verifie BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS verifie_par UUID REFERENCES utilisateurs(id),
ADD COLUMN IF NOT EXISTS date_verification TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS reference_liee VARCHAR(100);  -- Référence à une opération liée

-- Ajouter des champs pour la classification des lignes d'écritures
ALTER TABLE journal_lines
ADD COLUMN IF NOT EXISTS libelle_detaille TEXT,  -- Libellé plus détaillé de la ligne
ADD COLUMN IF NOT EXISTS centre_cout VARCHAR(20),  -- Centre de coût si applicable
ADD COLUMN IF NOT EXISTS projet VARCHAR(50);  -- Projet si applicable
...
CREATE TABLE budgets (
    ...
    compte_numero VARCHAR(20) NOT NULL REFERENCES plan_comptable(numero),
    montant_prevu NUMERIC(18,2) NOT NULL,
    montant_realise NUMERIC(18,2) DEFAULT 0,
    ecart NUMERIC(18,2) GENERATED ALWAYS AS (montant_prevu - montant_realise) STORED,
    compagnie_id UUID REFERENCES utilisateurs(id),
    statut VARCHAR(20) DEFAULT 'En cours' CHECK (statut IN ('En cours', 'Termine', 'Modifie', 'Archive')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Table pour les prévisions de trésorerie
CREATE TABLE prevision_tresorerie (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tresorerie_id UUID REFERENCES tresoreries(id),
    date_prevue DATE NOT NULL,
    type_flux VARCHAR(20) NOT NULL CHECK (type_flux IN ('Entree', 'Sortie')),
    montant_prevu NUMERIC(18,2) NOT NULL,
    description TEXT,
    categorie VARCHAR(50),  -- Ex: 'Vente', 'Achat', 'Salaire', 'Loyer', etc.
    compagnie_id UUID REFERENCES utilisateurs(id),
    statut VARCHAR(20) DEFAULT 'Prevu' CHECK (statut IN ('Prevu', 'Realise', 'Annule')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE alertes_systeme (
    ...
    description TEXT,
    type_alerte VARCHAR(30) NOT NULL CHECK (type_alerte IN ('Stock', 'Finance', 'Comptabilite', 'RessourcesHumaines', 'Operation', 'Autre')),
    priorite INTEGER DEFAULT 1 CHECK (priorite BETWEEN 1 AND 5),  -- 1 = basse, 5 = haute
    destinataire_id UUID REFERENCES utilisateurs(id),  -- Utilisateur destinataire
    statut VARCHAR(20) DEFAULT 'Nouveau' CHECK (statut IN ('Nouveau', 'Lu', 'Traite', 'Archive')),
    reference_entite VARCHAR(100),  -- Référence à l'entité concernée
    date_creation TIMESTAMPTZ NOT NULL DEFAULT now(),
    date_traitement TIMESTAMPTZ,
    compagnie_id UUID REFERENCES utilisateurs(id)
);

-- Mise à jour de la table journal_entries pour inclure plus de traçabilité
ALTER TABLE journal_entries
ADD COLUMN IF NOT EXISTS ip_origine VARCHAR(45),  -- IP de l'utilisateur lors de la création
ADD COLUMN IF NOT EXISTS poste_travail VARCHAR(100),  -- Poste de travail utilisé
ADD COLUMN IF NOT EXISTS session_id VARCHAR(100);  -- ID de la session utilisateur
...
-- 11) Ventes & Détails (liés à un gérant)
CREATE TABLE ventes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID REFERENCES clients(id) ON DELETE SET NULL,
    date_vente DATE NOT NULL,
...
-- 12) Inventaires (carburant et boutique)
...
-- 13) Dépenses courantes (hors achats/ventes)
...
-- 18) Bilan initial (depuis energix_additif.sql)
...
-- 19) Périodes comptables (depuis energix_additif.sql)
...
-- 20) Soldes comptables (depuis energix_additif.sql)
...
-- 21) Rapports financiers (depuis energix_additif.sql)
...
-- Table pour les écarts de soldes de tiers
