...
def get_mouvements_tresorerie_by_transfert_id(db: Session, current_user, transfert_id: uuid.UUID):
    """Récupère les mouvements de trésorerie liés à un transfert spécifique"""
    # Vérifier que le transfert appartient à la même compagnie que l'utilisateur
    transfert = db.query(TransfertTresorerieModel).join(
        TresorerieModel,
        TransfertTresorerieModel.tresorerie_source_id == TresorerieModel.id
    ).filter(
    ...
    # Récupérer les mouvements liés à ce transfert via la référence
    reference_transfert = f"TRANSFERT-{transfert_id}"
    mouvements = get_mouvements_tresorerie_by_reference(db, current_user, reference_transfert)
